...
# ========= GPU HASSLE!
# import torch
# # 1. Initialize CUDA and set memory limits
# if torch.cuda.is_available():
#     # Set memory fraction to 0.3 (30% of GPU memory)
...
# # 2. Set environment variable for memory management
# import os
# os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'

# # 3. Function to clear GPU cache
# def clear_gpu_memory():
#     import gc
#     gc.collect()
#     torch.cuda.empty_cache()

# # Call this before major operations
# clear_gpu_memory()
# # ========= 
...
def main():
	...
	if isinstance(tokenizer, tuple(MULTILINGUAL_TOKENIZERS)):
		...
		tokenizer.src_lang = "eng_Latn"#"data_args.source_lang"
		tokenizer.tgt_lang = "eng_Latn"#data_args.target_lang
	...
	def preprocess_function(examples):
		"""Preprocess the examples for NLLB training handling None values."""
		source_texts = []
		target_texts = []
		
		for translation in examples["translation"]:
			# Find the valid language pair (non-None values)
			valid_langs = [(lang, text) for lang, text in translation.items() if text is not None]
			
			if len(valid_langs) != 2:
				print(f"Warning: Found {len(valid_langs)} valid languages instead of 2, skipping example")
				continue
				
			# Sort to ensure eng_Latn is target
			valid_langs.sort(key=lambda x: x[0] == "eng_Latn")
			src_lang, src_text = valid_langs[0]
			tgt_lang, tgt_text = valid_langs[1]
			
			# Verify it's a valid source-target pair
			if tgt_lang != "eng_Latn":
				print(f"Warning: Target language is not eng_Latn: {tgt_lang}, skipping")
				continue
		...
		if not source_texts:
			raise ValueError("No valid translation pairs found in batch")
			
		# Print first few examples for verification
		print("\nFirst few preprocessed pairs:")
